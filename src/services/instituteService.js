...
async function list({ limit = 200, offset = 0 }) {
  const [rows] = await pool.query(
    `SELECT * FROM institutes ORDER BY name LIMIT ? OFFSET ?`,
...
async function get(id) {
  ...
  if (row && row.specialties && typeof row.specialties === "string") {
    try {
      ...
      console.error(
        `[AVISO] Falha ao parsear 'specialties' para o instituto ID ${id}. Conteúdo: "${row.specialties}". Tratando como array de item único.`
      );
...
async function update(id, data, user) {
  ...
  if (data.specialties !== undefined) {
    ...
    const specialtiesAsJson = Array.isArray(data.specialties)
  ...
  const [r] = await pool.query(
    `UPDATE institutes SET ${fields.join(", ")} WHERE id=?`,
    params
  );
  await logAction({
    userId: user?.id,
...
async function softDelete(id, user) {
  const before = await get(id); // Agora esta chamada é segura
  const [r] = await pool.query(
    `UPDATE institutes SET status='inactive' WHERE id=?`,
    [id]
  );
  await logAction({
    userId: user?.id,
