...
async function softDelete(id, user) { const before = await get(id); const [r] = await pool.query(`UPDATE institutes SET status='inactive' WHERE id=?`, [id]); await logAction({ userId: user?.id, action: 'DELETE', tableName: 'institutes', recordId: id, oldValues: before, newValues: { status: 'inactive' } }); return !!r.affectedRows; }
