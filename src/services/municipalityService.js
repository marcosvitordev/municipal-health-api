...
async function list({ limit = 200, offset = 0 }) {
  const [rows] = await pool.query(
    `SELECT * FROM municipalities ORDER BY name LIMIT ? OFFSET ?`,
...
async function update(id, data, user) {
  ...
  const [r] = await pool.query(
    `UPDATE municipalities SET ${fields.join(", ")} WHERE id=?`,
    params
  );
  await logAction({
    userId: user?.id,
...
async function softDelete(id, user) {
  const before = await get(id);
  const [r] = await pool.query(
    `UPDATE municipalities SET status='inactive' WHERE id=?`,
    [id]
  );
  await logAction({
    userId: user?.id,
