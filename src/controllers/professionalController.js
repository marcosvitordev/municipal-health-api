...
async function create(req, res, n) {
  try {
    res.status(201).json(await prof.create(req.body, req.user));
  } catch (e) {
    n(e);
  }
}
async function get(req, res, n) {
  try {
    const r = await prof.get(req.params.id);
...
async function update(req, res, n) {
  try {
    res.json({ ok: await prof.update(req.params.id, req.body, req.user) });
  } catch (e) {
    n(e);
  }
}
async function remove(req, res, n) {
  try {
    res.json({ ok: await prof.softDelete(req.params.id, req.user) });
  } catch (e) {
    n(e);
  }
}
