...
async function updateMe(req, res, next) {
    try {
        ...
        const ok = await users.update(req.user.id, { name, email, phone }, req.user);
        res.json({ ok });
    } catch (e) {
        next(e);
    }
...
async function updateMyPassword(req, res, next) {
    try {
        ...
        const ok = await users.changePassword(req.user.id, currentPassword, newPassword, req.user);
        res.json({ ok });
    } catch (e) {
        next(e);
    }
...
module.exports = { list, create, update, remove, getMe, updateMe, updateMyPassword };
