...
async function uploadDoc(req,res,next){
  try{
    const f = req.file;
    if(!f) return res.status(400).json({error:'Arquivo obrigatório (campo "file")'});
    ...
    const [r] = await pool.query(
      `INSERT INTO documents (request_id, type, name, file_path, file_size, mime_type, uploaded_by, description)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [request_id, type, name || f.originalname, key, f.size, f.mimetype, req.user.id, description||null]
    );
    await logAction({ userId: req.user.id, action: 'UPLOAD', tableName: 'documents', recordId: r.insertId, newValues: { request_id, type, name } });
