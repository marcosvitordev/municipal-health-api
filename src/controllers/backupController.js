...
async function startManualBackup(req, res, next) {
  try {
    ...
    backupService
      .createBackup(userInfo)
      .then((result) =>
        console.log(`Backup ${result.filename} criado com sucesso.`)
      )
      .catch((err) =>
        console.error(`Falha no processo de backup em background:`, err)
      );
    ...
    res
      ...
      .json({
        message:
          "Processo de backup iniciado em segundo plano. Verifique o histórico para o resultado.",
      });
  } catch (e) {
    next(e);
  }
